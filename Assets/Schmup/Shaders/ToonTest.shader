    ...
    SubShader
    {
        Pass
        {
            ...
            float4 frag (v2f input) : SV_Target
            {
                ...
                float3 lighting = ((diffuse + (specular*_SpecularMultiplier) + _AmbientStrength) * _LightColor0.rgb);

                float2 TiledMainUV = TRANSFORM_TEX(input.uv, _MainTex);
                float3 albedo = tex2D(_MainTex, TiledMainUV);
